...
class CandidateDelegates(BaseModel):
    ...
    def pledged_delegates_pct(self):
        return 100 * ((self.delegates_count - self.superdelegates_count) / (float(self.party_need) + 450))

    def superdelegates_pct(self):
        return 100 * (self.superdelegates_count / (float(self.party_need) + 450))

    def delegates_pct(self):
        return 100 * (self.delegates_count / (float(self.party_need) + 450))

    def needed_bar_position(self):
        return  100 * (float(self.party_need) / (float(self.party_need) + 450))
